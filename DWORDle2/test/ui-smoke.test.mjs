...
import { pidForNumber, pidLabel, todayPID } from "../js/core/problems.js?v=20260806-a";
...
try {
  ...
  assert.equal(
    await page.evaluate(async () => {
      const mod = await import("./js/audio/sound.js?v=20260806-a");
  ...
  await page.evaluate(async () => {
    const mod = await import("./js/core/settings.js?v=20260806-a");
  ...
  {
    await page.evaluate(async () => {
      const { showHelpModal } = await import("./js/ui/help.js?v=20260806-a");
  ...
  const normalPopFlag = await page.evaluate(async () => {
    ...
    const { renderResultCanvas } = await import("./js/ui/snapshot.js?v=20260806-a");
  ...
  await page.evaluate(async () => {
    const { setAppMode } = await import("./js/ui/app.js?v=20260806-a");
    setAppMode("uso");
  });
  ...
  const savedImageFlagPixels = await page.evaluate(async ({ guessedWord }) => {
    const settings = await import("./js/core/settings.js?v=20260806-a");
    const { renderResultCanvas } = await import("./js/ui/snapshot.js?v=20260806-a");
  ...
  await page.evaluate(async ({ times, playCount }) => {
    const records = await import("./js/core/records.js?v=20260806-a");
  ...
  await page.evaluate(async () => {
    ...
    const { pidForNumber } = await import("./js/core/problems.js?v=20260806-a");
  ...
  assert.equal(
    await page.evaluate(async ({ pid }) => {
      const { confirmAndStart } = await import("./js/ui/game-screen.js?v=20260806-a");
  ...
  await page.evaluate(async ({ pid }) => {
    const [{ Logic: BrowserLogic }, records, app] = await Promise.all([
      import("./js/core/logic.js?v=20260806-a"),
      import("./js/core/records.js?v=20260806-a"),
      import("./js/ui/app.js?v=20260806-a"),
    ]);
    const logic = new BrowserLogic(pid);
  ...
  await page.evaluate(async () => {
    const app = await import("./js/ui/app.js?v=20260806-a");
  ...
  // EXTRA SHOT 成功: 最後の 1 枚だけ溜め、結果・保存画像を通常盤面の下へ表示する
  {
    ...
    assert.equal(
      await successPage.evaluate(async () => (await import("./js/config.js?v=20260806-a")).FX.extraShot.lastTilePauseMs),
      720,
    ...
    const crownGeometry = await successPage.evaluate(async () => {
      const { CROWN_POINT_COUNT, CROWN_VALLEY_COUNT, crownPoints } = await import("./js/ui/crown.js?v=20260806-a");
    ...
    const snapshotExtraShot = await successPage.evaluate(async () => {
      ...
      const { Logic } = await import("./js/core/logic.js?v=20260806-a");
      const { renderResultCanvas } = await import("./js/ui/snapshot.js?v=20260806-a");
      const settings = await import("./js/core/settings.js?v=20260806-a");
    ...
    await successPage.evaluate(async () => {
      (await import("./js/ui/toast.js?v=20260806-a")).extraShotUnlockCelebration();
    });
  ...
  const flightsBeforeLeave = await shortPage.evaluate(async () =>
    (await import("./js/fx/effects.js?v=20260806-a")).activeTileFlightCount()
  );
  ...
  await page.evaluate(async () => {
    const { showConsentBanner } = await import("./js/ui/consent-banner.js?v=20260806-a");
    showConsentBanner();
  });
  ...
  try {
    ...
    await freshPage.evaluate(async () => {
      const { showConsentBannerAfterModals } = await import("./js/ui/consent-banner.js?v=20260806-a");
  ...
  try {
    ...
    const badgeInfo = await cardPage.evaluate(async () => {
      const [cardMod, achMod] = await Promise.all([
        import("./js/ui/player-card.js?v=20260806-a"),
        import("./js/core/achievements.js?v=20260806-a"),
      ]);
  ...
  try {
    ...
    assert.equal(
      await moodPage.evaluate(async () => {
        const mod = await import("./js/core/activity.js?v=20260806-a");
