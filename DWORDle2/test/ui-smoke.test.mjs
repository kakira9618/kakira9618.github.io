...
import { ACHIEVEMENTS, HIDDEN_ACHIEVEMENTS, NORMAL_ACHIEVEMENTS } from "../js/core/achievements.js?v=20260803-e";
// 隠し要素の文字列はテストにも平文で置かない（js/core/secret.js 参照）
import { reveal } from "../js/core/secret.js?v=20260803-e";
import { pidForNumber, pidLabel, todayPID } from "../js/core/problems.js?v=20260803-e";
...
// バージョン表示のハッシュ（DWORDle2 を最後に変更したコミット）と sw.js の整合
{
  ...
  const { SOURCE_HASH } = await import("../js/version.js?v=20260803-e");
...
{
  const { APP_VERSION } = await import("../js/config.js?v=20260803-e");
...
try {
  ...
  assert.equal(
    await page.evaluate(async () => {
      const mod = await import("./js/audio/sound.js?v=20260803-e");
  ...
  assert.deepEqual(
    await page.evaluate(async () => {
      const s = (await import("./js/core/settings.js?v=20260803-e")).getSettings();
  ...
  await page.evaluate(async () => {
    const mod = await import("./js/core/settings.js?v=20260803-e");
  ...
  {
    await page.evaluate(async () => {
      const { showHelpModal } = await import("./js/ui/help.js?v=20260803-e");
  ...
  assert.equal(await switches.count(), 5, "The Display tab should expose its five switches");
  ...
  assert.deepEqual(
    displayRowBorders,
    ["solid", "solid", "solid", "solid", "solid", "solid", "none"],
  ...
  const normalPopFlag = await page.evaluate(async () => {
    ...
    const { renderResultCanvas } = await import("./js/ui/snapshot.js?v=20260803-e");
  ...
  await page.evaluate(async () => {
    const { setAppMode } = await import("./js/ui/app.js?v=20260803-e");
    setAppMode("uso");
  });
  ...
  const savedImageFlagPixels = await page.evaluate(async ({ guessedWord }) => {
    const settings = await import("./js/core/settings.js?v=20260803-e");
    const { renderResultCanvas } = await import("./js/ui/snapshot.js?v=20260803-e");
  ...
  const shareTextOf = async (game) => {
    const startTime = await page.evaluate(async (fixture) => {
      const [{ Logic: BrowserLogic }, records, { pidForNumber, todayPID }] = await Promise.all([
        import("./js/core/logic.js?v=20260803-e"),
        import("./js/core/records.js?v=20260803-e"),
        import("./js/core/problems.js?v=20260803-e"),
      ]);
  ...
  await page.evaluate(async () => {
    ...
    const { pidForNumber } = await import("./js/core/problems.js?v=20260803-e");
  ...
  assert.equal(
    await page.evaluate(async ({ pid }) => {
      const { confirmAndStart } = await import("./js/ui/game-screen.js?v=20260803-e");
  ...
  await page.evaluate(async ({ pid }) => {
    const [{ Logic: BrowserLogic }, records, app] = await Promise.all([
      import("./js/core/logic.js?v=20260803-e"),
      import("./js/core/records.js?v=20260803-e"),
      import("./js/ui/app.js?v=20260803-e"),
    ]);
    const logic = new BrowserLogic(pid);
  ...
  await page.evaluate(async () => {
    const app = await import("./js/ui/app.js?v=20260803-e");
  ...
  // EXTRA SHOT 成功: 最後の 1 枚だけ溜め、結果・保存画像を通常盤面の下へ表示する
  {
    ...
    assert.equal(
      await successPage.evaluate(async () => (await import("./js/config.js?v=20260803-e")).FX.extraShot.lastTilePauseMs),
      720,
    ...
    const crownGeometry = await successPage.evaluate(async () => {
      const { CROWN_POINT_COUNT, CROWN_VALLEY_COUNT, crownPoints } = await import("./js/ui/crown.js?v=20260803-e");
    ...
    const snapshotExtraShot = await successPage.evaluate(async () => {
      ...
      const { Logic } = await import("./js/core/logic.js?v=20260803-e");
      const { renderResultCanvas } = await import("./js/ui/snapshot.js?v=20260803-e");
      const settings = await import("./js/core/settings.js?v=20260803-e");
    ...
    await successPage.evaluate(async () => {
      (await import("./js/ui/toast.js?v=20260803-e")).extraShotUnlockCelebration();
    });
  ...
  const flightsBeforeLeave = await shortPage.evaluate(async () =>
    (await import("./js/fx/effects.js?v=20260803-e")).activeTileFlightCount()
  );
  ...
  await page.evaluate(async () => {
    const { bgmUnlockCelebration } = await import("./js/ui/toast.js?v=20260803-e");
  ...
  await page.evaluate(async () => {
    const { achievementCelebration } = await import("./js/ui/toast.js?v=20260803-e");
  ...
  await page.evaluate(async () => {
    const { showConsentBanner } = await import("./js/ui/consent-banner.js?v=20260803-e");
    showConsentBanner();
  });
  ...
  try {
    ...
    await freshPage.evaluate(async () => {
      const { showConsentBannerAfterModals } = await import("./js/ui/consent-banner.js?v=20260803-e");
  ...
  try {
    ...
    const ranks = await cardPage.evaluate(async () => {
      const mod = await import("./js/ui/player-card.js?v=20260803-e");
      const pick = (stats) => {
    ...
    const badgeInfo = await cardPage.evaluate(async () => {
      const [cardMod, achMod] = await Promise.all([
        import("./js/ui/player-card.js?v=20260803-e"),
        import("./js/core/achievements.js?v=20260803-e"),
      ]);
    ...
    await cardPage.evaluate(async () => {
      const mod = await import("./js/core/achievements.js?v=20260803-e");
      const all = {};
  ...
  try {
    ...
    assert.equal(
      await moodPage.evaluate(async () => {
        const mod = await import("./js/core/activity.js?v=20260803-e");
  ...
  // 設定「判定マーク」: ON にすると判定済みタイルの右下に 正解=● / 位置違い=△ / 不使用=× が付き、
  // 色以外でも判定を区別できる（WCAG 2.1「1.4.1 色の使用」）
  {
    const pid = 1; // 答えは point / touch。crane は 位置違い/不使用/不使用/正解/不使用 と 3 種すべて出る
    const logic = new Logic(pid);
    assert.deepEqual(logic.queryWord("crane"), ["used", "unused", "unused", "correct", "unused"]);
    const symbolsPage = await browser.newPage({ viewport: { width: 390, height: 844 }, locale: "ja-JP" });
    await symbolsPage.addInitScript(({ puzzleId }) => {
      localStorage.setItem("dwordle2.settings", JSON.stringify({
        theme: "classic",
        sfx: false,
        bgm: false,
    ...
    await symbolsPage.goto(`${baseUrl}#/game`, { waitUntil: "networkidle" });
    await passGate(symbolsPage);
    await symbolsPage.locator("#screen-game.active .row").last().waitFor();
    assert.equal(
      await symbolsPage.evaluate(() => document.body.classList.contains("state-symbols")),
      true,
      "stateSymbols: true should set the state-symbols body class"
    );
    await symbolsPage.keyboard.type("crane");
    await symbolsPage.keyboard.press("Enter");
    await symbolsPage.waitForFunction(
      () =>
        document.querySelectorAll(
          "#screen-game.active #board .row:first-child .tile.state-correct, " +
            "#screen-game.active #board .row:first-child .tile.state-used, " +
            "#screen-game.active #board .row:first-child .tile.state-unused"
        ).length === 5,
      null,
      { timeout: 8000 }
    );
    const marks = await symbolsPage.evaluate(() =>
      [...document.querySelectorAll("#screen-game.active #board .row:first-child .tile")].map(
        (tile) => getComputedStyle(tile, "::after").content
      )
    );
    assert.deepEqual(
      marks,
      ['"△"', '"×"', '"×"', '"●"', '"×"'],
      "revealed tiles should carry the feedback symbol matching their state"
    );
    // OFF に戻すと記号は消える（既定 OFF の見た目が変わっていないことの確認を兼ねる）
    await symbolsPage.evaluate(() => document.body.classList.remove("state-symbols"));
    const marksOff = await symbolsPage.evaluate(() =>
      [...document.querySelectorAll("#screen-game.active #board .row:first-child .tile")].map(
        (tile) => getComputedStyle(tile, "::after").content
      )
    );
    assert.deepEqual(marksOff, Array(5).fill("none"), "symbols should disappear when the setting is off");
    await symbolsPage.close();

    // 結果画面の小タイル (.rcell) にも同じ記号が付く（分析・遊び方も同じクラスを共有）。
    // 判定のない答え行のタイルには付かない
    const resultPage = await browser.newPage({ viewport: { width: 390, height: 844 }, locale: "ja-JP" });
    await resultPage.addInitScript(({ puzzleId }) => {
      localStorage.setItem("dwordle2.settings", JSON.stringify({
        theme: "classic",
        sfx: false,
        bgm: false,
      ...
      localStorage.setItem("dwordle2.history", JSON.stringify([{
        version: "2.0.0",
        startTime: 1_800_000_401,
        endTime: 1_800_000_460,
        gameMode: "normal",
        problemID: puzzleId,
        guessWord: ["crane", "point"],
        clear: true,
      }]));
      localStorage.setItem("dwordle2.achievements", "{}");
      localStorage.setItem("dwordle2.achievements.reconcileVersion", "99");
    ...
    await resultPage.goto(`${baseUrl}#/result/normal/1800000401`, { waitUntil: "networkidle" });
    await passGate(resultPage);
    await resultPage.locator("#screen-result.active .result-grid .rrow").first().waitFor();
    const resultMarks = await resultPage.evaluate(() =>
      [...document.querySelectorAll("#screen-result.active .result-grid .rrow")].map((row) =>
        [...row.querySelectorAll(".rcell")].map((cell) => getComputedStyle(cell, "::after").content)
      )
    );
    assert.deepEqual(
      resultMarks,
      ...
      "result-screen cells should carry the same feedback symbols"
    );
    const answerMarks = await resultPage.evaluate(() =>
      [...document.querySelectorAll("#screen-result.active .answer-row .rcell")].map(
        (cell) => getComputedStyle(cell, "::after").content
      )
    );
    assert.deepEqual(answerMarks, Array(10).fill("none"), "answer tiles without feedback should stay unmarked");
