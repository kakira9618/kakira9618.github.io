...
// 未選択: 地域を問わずバナー対象だが、タグも dataLayer も作らない。
{
  ...
  const analytics = await import(`../js/core/analytics.js?case=unselected&v=20260725-b`);
...
// 初回拒否: Google には何も積まず、構造化した選択と時刻を端末に保存する。
{
  ...
  const analytics = await import(`../js/core/analytics.js?case=decline&v=20260725-b`);
  analytics.initAnalytics();
...
// 明示同意: default denied → analytics granted の順に積み、初めてタグとイベントを準備する。
{
  ...
  const analytics = await import(`../js/core/analytics.js?case=accept&v=20260725-b`);
  analytics.initAnalytics();
...
// 現行ポリシーへ同意済みなら、起動時にタグを読み込む。
{
  ...
  const analytics = await import(`../js/core/analytics.js?case=current-grant&v=20260725-b`);
  assert.equal(analytics.needsConsentPrompt(), false);
  analytics.initAnalytics();
...
// 旧実装の granted は新しい全地域共通ポリシーへの同意ではないため、再確認する。
{
  ...
  const analytics = await import(`../js/core/analytics.js?case=legacy-grant&v=20260725-b`);
...
// 旧実装の denied は尊重し、勝手に再確認・計測しない。
{
  ...
  const analytics = await import(`../js/core/analytics.js?case=legacy-denied&v=20260725-b`);
...
// 同意後の撤回: denied をタグへ伝え、Cookie を消し、以後のゲームイベントを止める。
{
  ...
  const analytics = await import(`../js/core/analytics.js?case=withdraw&v=20260725-b`);
  analytics.initAnalytics();
  analytics.setAnalyticsConsent(true);
...
// 同意直後、アイドル読み込み前に撤回した場合もタグを取得しない。再許可時は読み込める。
{
  ...
  const analytics = await import(`../js/core/analytics.js?case=idle-race&v=20260725-b`);
  analytics.initAnalytics();
  analytics.setAnalyticsConsent(true);
  analytics.setAnalyticsConsent(false);
  idleCallbacks.shift()();
...
for (const [name, options] of [
  ...
  const analytics = await import(`../js/core/analytics.js?case=${name}&v=20260725-b`);
