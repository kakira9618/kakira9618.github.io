...
import { APP_VERSION, UI, FX } from "../config.js?v=20260803-a";
import { Logic, CELL, displayResultForMode } from "../core/logic.js?v=20260803-a";
import { MODES, saveCurrentGame, clearCurrentGame, getCurrentGame, addFinishedGame, addDiscardedGame, isAlreadyPlayed, getHistory, getExtraShot } from "../core/records.js?v=20260803-a";
import { NEW_ERA, isClassicPID, isDailyPID, numberPrefix, pidLabel, todayPID } from "../core/problems.js?v=20260803-a";
import { checkOnGameFinish } from "../core/achievements.js?v=20260803-a";
import { addReloadBlocker } from "../core/critical-update.js?v=20260803-a";
import { registerScreen, navigate, redirect, getAppMode, currentScreenName } from "./app.js?v=20260803-a";
import { toast, achievementCelebration, bgmUnlockCelebration, themeUnlockCelebration, extraShotUnlockCelebration } from "./toast.js?v=20260803-a";
import { isExtraShotEnabled, claimExtraShotUnlockNotice } from "../core/extra-shot.js?v=20260803-a";
import { playExtraShotCutin, playDoubleClearCutin, cancelExtraShotFx } from "./extra-shot-fx.js?v=20260803-a";
import { bgmTracksUnlockedBy, playSfx } from "../audio/sound.js?v=20260803-a";
import { hiddenThemesUnlockedBy } from "../core/settings.js?v=20260803-a";
import { burstAtElement, cancelTileFlights, winBurst, colorForState, flyInTiles } from "../fx/effects.js?v=20260803-a";
import { showHelpModal, hasSeenHelp } from "./help.js?v=20260803-a";
import { trackEvent } from "../core/analytics.js?v=20260803-a";
import { soundToggleButton } from "./sound-toggle.js?v=20260803-a";
import { icon } from "./icons.js?v=20260803-a";
import { tr } from "../core/i18n.js?v=20260803-a";
import { getSettings } from "../core/settings.js?v=20260803-a";
import { shouldReduceMotion } from "../core/motion.js?v=20260803-a";
import { announce, feedbackName, rowAriaLabel, tileAriaLabel } from "./a11y.js?v=20260803-a";
...
// タッチ/ペンで処理済みのキー。あとから届く合成 click をここで打ち消す。
// 設計の要点（いずれも実機で二重入力になった経緯がある）:
//  - キーごとに持つ: 直前の 1 キーだけだと、P → O と速く打ったときに P の合成 click が
//    O のあとに届いて素通りし "POP" になる
//  - 件数を数えない: iOS は 1 タップに対して複数の合成 click を出すことがある
//  - event.timeStamp を見ない: 合成イベントの時刻は基準がずれることがある。
//    タイマーで期限を切る
const TOUCH_CLICK_WINDOW_MS = 1200;
const touchHandledKeys = new Set(); // タッチで処理済みのキー
const touchHandledTimers = new Map(); // key -> 期限切れタイマー
...
function markTouchHandled(key) {
  clearTimeout(touchHandledTimers.get(key));
  touchHandledKeys.add(key);
  touchHandledTimers.set(
    key,
    setTimeout(() => {
      touchHandledKeys.delete(key);
      touchHandledTimers.delete(key);
    }, TOUCH_CLICK_WINDOW_MS)
  );
}

// そのキーを直近でタッチ処理していれば、この click は合成 click なので捨てる。
// キーボード・支援技術からの click は、直前にタッチが無いのでそのまま通る。
function isSyntheticTouchClick(key) {
  return touchHandledKeys.has(key);
}
      ...
      const btn = el(
        "button",
        {
          ...
          onclick: (event) => {
            ...
            if (isSyntheticTouchClick(k)) return;
            handleKey(k);
          },
        },
        label
...
function installBoardDragScroll() {
  ...
  boardScrollEl.addEventListener("pointermove", (e) => {
    ...
    // clientY は大画面での全体拡大（--app-zoom）適用後の px、scrollTop は適用前の
    // 座標系。倍率で割り、指の移動量と盤面の移動量を画面上で一致させる
    boardScrollEl.scrollTop = startScroll - delta / effectiveZoom(boardScrollEl);
    e.preventDefault();
  });
  const stop = (e) => {
    if (e.pointerId !== pointerId) return;
...
async function askLeaveGame() {
  const { showModal } = await import("./modal.js?v=20260803-a");
...
async function requestBackToTitle() {
  ...
  const { confirmModal } = await import("./modal.js?v=20260803-a");
