// FINAL ANSWER の演出（突入カットイン / DOUBLE CLEAR セレブレーション）。
// DOM オーバーレイ + CSS アニメーションで構成し、テーマ×モード
// （cyber / classic / pop × DWORDle / DWORDlie）ごとの見た目の差は
// style.css の「FINAL ANSWER」セクションが body.theme-* / body.mode-uso で出し分ける。
// 表示時間は config.js の FX.finalAnswer にまとまっている（CSS と同期）。
...
export function cancelFinalAnswerFx() {
...
// 突入カットイン。「FINAL ANSWER」の帯が画面を横切り、入力可能になるタイミングで resolve する。
export function playFinalAnswerCutin(isUso) {
  return new Promise((resolve) => {
    if (shouldReduceMotion()) {
      setTimeout(resolve, FX.finalAnswer.cutinReducedMs);
      return;
    }
    const sub = isUso
    ...
    const overlay = el(
      ...
      el(
        ...
        el(
          ...
          el("div", { class: "fa-cutin-text" }, "FINAL ANSWER"),
          el("div", { class: "fa-cutin-sub" }, sub)
        )
      )
    );
    showOverlay(overlay, FX.finalAnswer.cutinMs, resolve);
  });
}
...
export function playDoubleClearCutin() {
  ...
  showOverlay(overlay, FX.finalAnswer.doubleClearMs);
}
