...
`npm test` = ユニット（`test/*.test.mjs`、数秒）+ UI スモーク（`test/ui-smoke.test.mjs`）。
UI スモークはシナリオを `test/ui/*.mjs` に分けてあり、**触った範囲だけ**を回せる。

```
npm run test:ui                       # 全部
npm run test:ui -- card history       # ファイル名 / シナリオ名に語を含むものだけ
npm run test:ui:list                  # 一覧（ID は「ファイル名/シナリオ名」）
node test/ui-smoke.test.mjs --jobs 1  # 並列数（既定 2。演出の時間計測が揺れるときは 1 に）
```

作業中は関係するシナリオだけを回し、コミット前に全部を回す。

各シナリオは自分のブラウザコンテキストを持つ（`test/ui/harness.mjs` の `openPage`）。
localStorage のシードは `VETERAN`（段階解放と初回案内を済ませた状態）に `storage` で上書きする。
初回起動を再現するときは `fresh: true`。新しい「既読フラグ」を足したら `VETERAN` に 1 か所足せばよい。

UI スモークに入れるもの・入れないもの:

- 入れる: 画面遷移、ダイアログの出る条件、保存データの形、解放条件、a11y（axe の serious 以上）、
  オフライン動作、過去に実際に起きた回帰。
- 入れない: 色・座標・フォントサイズ・アニメーション名などの CSS の値そのもの、
  演出のタイミング計測、「以前の文言が残っていないこと」のような仕様変更の差分確認。
  仕様を変えたときはテストも**最終状態だけ**を見る形に書き換え、変更前後の対比は残さない。
- 1 局の判定や集計のように DOM が要らないものはユニットテストへ。
